...
class Corpus:
    ...
    def __init__(self, filePath, maxSentenceWordsNum=-1, id2word=None, word2id=None, wordNum=None, tfidf=False, QIDF=None, AIDF=None, testSize=0.2, isCharVec=False):
        ...
        with open(filePath,'r',encoding='utf8') as f:
            txt = self._purify(f.readlines())#去掉停用词
            data = [i.split('\t') for i in txt]#分割句子
            #判断是否是字符，使用jieba进行分词
            if isCharVec:
                data = [[[c for c in i[0]], [c for c in i[1]]] for i in data]
            else:
                data = [[jieba.lcut(i[0]), jieba.lcut(i[1])] for i in data]
            data = [i for i in data if (len(i[0])<maxSentenceWordsNum and len(i[1])<maxSentenceWordsNum) or maxSentenceWordsNum==-1]#去掉句子长度大于最长长度的句子
        ...
        try:
            ...
            chatDataId = [[[self.word2id[w] for w in qa[0] if w in self.id2word],[self.word2id[w] for w in qa[1] if w in self.id2word]] for qa in self.chatDataWord]#将切分后句子的单词转化成对应的id编号，因为神经网络只能接受数字不能处理文本
        self._QALens(chatDataId)#求出每个句子的长度，和一个batch中句子的最大长度
        self.maxSentLen = max(maxSentenceWordsNum, self.AMaxLen)
        self.QChatDataId, self.AChatDataId = [qa[0] for qa in chatDataId], [qa[1] for qa in chatDataId]#将问句的id编号和回答的id编号分开
        self.totalSampleNum = len(data)#句子样本总数
        #定义如果使用tfidf 手动实现
        if tfidf: #公式：IDF(t）=log_e(句子总数/含有单词t的句子总数），QIDF为问句的IDF，AIDF为回答的IDF
    ...
    def train(sentences, model_out_put_path):
        print("开始训练")
        model = word2vec.Word2Vec(sentences=sentences, size=GrobalParament.train_size,
                                  window=GrobalParament.train_window, min_count=20)
...
# a=Corpus("..\data\qingyun1.tsv")
# #
# # d,v,x,u =a.random_batch_data_stream(batchSize=128, isDataEnhance=False, dataEnhanceRatio=0.2, type='train')
# a.one_epoch_data_stream()