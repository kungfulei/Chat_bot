...
#定义贪婪搜索，inference时使用
    def predictByGreedySearch(self, inputSeq, maxAnswerLength=32, showAttention=False, figsize=(12,6)):
     ...
     #定义相关的层，确定相应的encoder，确定隐层
        d = int(self.encoderRNN.bidirectional)+1#加一是从一开始的
        ...
        hidden = hidden[-d*self.decoderRNN.numLayers::2].contiguous()# hidden为什么是两层？
    ...
    def predictByBeamSearch(self, inputSeq, beamWidth=10, maxAnswerLength=32, alpha=0.7, isRandomChoose=False, allRandomChoose=False, improve=True, showInfo=False):
        ...
        for i in range(1, maxAnswerLength):
           ...
           #计算过程，主要算概率，算路径上的最大概率
            ...
            hidden = hidden[:, idFrom, :]#hidden取值取idFrom这部分
            ...
            if beamWidth<1:  #直到搜索宽度为0
                break
        lens = [i.index(eosToken) if eosToken in i else maxAnswerLength for i in Y.tolist()]#长度，如果有终止符，及取终止符索引，如果没有就区最大的长度
...
def _calculate_loss(encoderRNN, decoderRNN, X, XLens, Y, YLens, teacherForcingRatio, device):
    ...
    encoderOutput, hidden = encoderRNN(X[indices], XLens, hidden)#降序处理，把X根据句子长度，从长到短排列，输入模型，为了后面的 pack_padded_sequence处理，pack_padded_sequence要求输入从长到短
    encoderOutput, hidden = encoderOutput[desortedIndices], hidden[-d*decoderRNN.numLayers::d, desortedIndices, :] #还原成原来的句子顺序。#hidden[:decoderRNN.numLayers, desortedIndices, :]
